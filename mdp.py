...
class MDP:
    ...
    def __init__(self, init, actlist, terminals, gamma, transitions=None, reward=None, states=None):
        if not (0 < gamma <= 1):
...
class GridMDP(MDP):
    ...
    def __init__(self, grid, terminals, gamma=1.0, intended = 0.8, left = 0.1, right = 0.1, init=(0, 0)):
    ...
    def calculate_T(self, state, action):
        if action:
            return [(self.intended, self.go(state, action)),
                    (self.right, self.go(state, turn_right(action))),
                    (self.left, self.go(state, turn_left(action)))]
        else:
            return [(0.0, state)]
...
def value_iteration(mdp, epsilon=0.00001):
    ...
    print('discount factor used in value iteration is {}'.format(mdp.gamma))
    sum= 0
    sum_a = []

    iterationNo = 1
    while True:
        print("interationb number is {}".format(iterationNo))
        U = U1.copy()
        delta = 0
        for s in mdp.states:   
        ...
        iterationNo += 1
           ...
           # U1[s] =  max(sum(p *(R(s1)+ gamma*U[s1]) for (p, s1) in T(s, a))
            #                           for a in mdp.actions(s))
...
def expected_utility(a, s, U, mdp):
  ...
  #  return sum(p * U[s1] for (p, s1) in mdp.T(s, a))
    return sum(p * (mdp.R(s1)+U[s1]) for (p, s1) in mdp.T(s, a))
...
def policy_iteration(mdp):
    ...
    print('discount factor used in policy iteration is {}'.format(mdp.gamma))
...
def policy_evaluation(pi, U, mdp, k=40):
    """Return an updated utility mapping U from each state in the MDP to its
    utility, using an approximation (modified policy iteration)."""

    R, T, gamma = mdp.R, mdp.T, mdp.gamma
    for i in range(k):
        print("interation number is {}/40".format(i))
        for s in mdp.states:
            #U[s] = R(s) + gamma * sum(p * U[s1] for (p, s1) in T(s, pi[s]))
            U[s] =  sum(p *(R(s1) + gamma * U[s1]) for (p, s1) in T(s, pi[s]))
            U[(3,1)] = R((3,1))
            U[(3,2)] = R((3,2))
            print (s, U[s])
    return U
